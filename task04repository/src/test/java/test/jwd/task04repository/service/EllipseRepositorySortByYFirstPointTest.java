...
import by.jwd.task04repository.service.specification.impl.SortEllipseByYFirstPointSpecificationImpl;

@RunWith(JUnitPlatform.class)
@SelectPackages("test.jwd.task04repository.service")

class EllipseRepositorySortByYFirstPointTest {
	...
	void testSortByXFirstPoint(List<IEllipse> actual, List<IEllipse> expected) {
		Assertions.assertEquals(actual, expected);
	}

	static Stream<Arguments> dataProvider() throws ServiceException {

		SortSpecification<IEllipse> specification = new SortEllipseByYFirstPointSpecificationImpl<>();
