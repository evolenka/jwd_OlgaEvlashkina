...
public class CreateAndDeleteMembershipTypeTest {
	
	static Logger logger = LogManager.getLogger(CreateAndDeleteMembershipTypeTest.class);
	...
	@Test(description = "createType")

	public void testCreateType() throws DaoException {
		...
		MembershipType type6 = new MembershipType ();
		type6.setTitle("NEW");
		type6.setMaxClassQuantity(9);
		type6.setPrice(200.0);
		...
		factory.getMembershipDao().createType(type6);
	...
	public void testDeleteType() throws DaoException {
		...
		Connection connection = ConnectionPool.getInstance().getConnection();
		DaoFactory factory = new DaoFactory (connection);
		List<MembershipType> all = factory.getMembershipDao().readAllTypes();
		MembershipType lastAdded = all.get(all.size()-1);
		factory.getMembershipDao().deleteType(lastAdded.getId());
		List<MembershipType> actual = factory.getMembershipDao().readAllTypes();
		assertEquals(actual, expected);
	}
}