...
<body>
	<div class=page>
		...
		<table class="table mx-auto">
			...
			<c:forEach var="plannedVisits" items="${plannedVisits}">
				<tr>
					<td><c:out value="${plannedVisits.danceclass.date}" /></td>
					<td><c:out value="${plannedVisits.danceclass.schedule.time}" /></td>
					<td><c:out
							value="${plannedVisits.danceclass.schedule.group.title}" /></td>
					<td><c:out
							value="${plannedVisits.danceclass.schedule.group.level}" /></td>
					<td><c:out value="${plannedVisits.membership.startDate}" /></td>
					<td><c:out value="${plannedVisits.membership.endDate}" /></td>
					<td><c:out value="${plannedVisits.membership.type}" /></td>
				</tr>
				<form method="post" action="action">
					<button type="submit" class="btn btn-light dropdown-item"
						data-toggle="dropdown" name="command" value="CANCELVISIT">
						<fmt:message key="cancelvisit" bundle="${ rb }" />
					</button>
				</form>
			</c:forEach>
		</table>
